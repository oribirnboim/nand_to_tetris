...
class Parser:
    ...
    def symbol(self) -> str:
        ...
        # Your code goes here!
        command = self.commands[self.current_line]
        if self.command_type() == "A_COMMAND": return command[1:]
        return command[1:-1]
    ...
    def comp(self) -> str:
        ...
        if eq_index >= 0:
            if jump_index < 0: return command[eq_index+1:]
            return command[eq_index+1:jump_index]
        if jump_index >= 0:
            return command[:jump_index]
        return command
